...
```{r mini batch cICA with Xc split}
minibatch_cICA = function (X, M = dim(X)[1], Win = diag(M), tol = 1e-04, maxit = 20, 
  ...
  wlik = -Inf
  rm(list = c("Win"))
  Xc_split = split_matrix(Xc)
    
  for (i in 1:length(Xc_split)){
    # Pre-processing Xc, considering to turn them into data frame
    Xc_mini = Xc_split[[i]]
    if (M > dim(Xin)[1]) {
      stop("Number of sources must be less or equal than number \n  of variables")
    }
...
S=rbind(S1,S2,S3)
X = A %*% S
cica = minibatch_cICA(X,tol=0.001)
